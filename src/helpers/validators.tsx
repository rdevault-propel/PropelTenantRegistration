export function validateEmailAddress(emailAddress: string): string {
  ...
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  return regex.test(emailAddress.trim()) ? '' : 'Invalid email address'
}

export function validateNumberOfUserLicenses(numberOfUserLicenses: number, min?: number) {
  if (numberOfUserLicenses === undefined || numberOfUserLicenses === null || isNaN(numberOfUserLicenses))
...
export function validatePhoneNumber(phoneNumber: string): string {
  ...
  const re = /^\+?[\d\s\-()]{7,15}$/
  return re.test(phoneNumber.trim()) ? '' : 'Invalid phone number'
}
