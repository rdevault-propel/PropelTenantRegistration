...
export function Registration() {
  ...
  const updateTenant = useCallback(<K extends keyof Tenant>(key: K, value: Tenant[K]) => {
    setTenant((prev) => ({ ...prev, [key]: value }))
  }, [])

  const steps: Step[] = [
    {
      element: <ConnectSignup tenant={tenant} updateTenant={updateTenant} setStep={setStep} />
    },
    {
      element: <ConnectPlan tenant={tenant} updateTenant={updateTenant} setStep={setStep} />
    },
    {
      element: <ConnectBilling tenant={tenant} setStep={setStep} />
    }
  ]
